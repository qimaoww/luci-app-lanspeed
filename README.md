...
- **配置页面**：LuCI 内置“实时状态”和“LAN Speed 配置”两个页签，速率采集、连接数采集、活跃客户端阈值和接口配置可分开调整，并由页面底部的统一按钮一次保存和重载；NSS 设备会显示 NSS 专属说明。
...
| 值 | 行为 |
|---|---|
| `auto` | 默认模式。普通设备优先 BPF；NSS ECM/PPE 设备使用 NSS sync / CT-Netlink 作为稳定来源，NSS-direct 有有效 flow 时补充；检测到 dae/daed 进程且 BPF 可用时立即优先使用 BPF。 |
...
dae 或 daed 进程启动后，daemon 会在下一次采集周期直接刷新 `/proc` 进程状态，并优先使用 LAN 边缘 BPF 作为实时测速来源。NSS 设备若缺少 BPF 包、对象或有效挂载，会按自动模式回退 NSS sync，并显示 `nss_dae_bpf_fallback_may_be_inaccurate` 告警。
...
| `lanspeedd-bpf` | SDK 编译的 tc/eBPF 对象和 BPF 运行时依赖（含 ct_lookup + seen_tuples 去重 map）；选择 LuCI 应用且构建配置提供 `HAS_BPF_TOOLCHAIN` 时默认选中 |

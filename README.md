...
本项目按平台严格分开速率口径：x86_64 只提供 TC-BPF；检测到 Qualcomm NSS/ECM 的 aarch64 设备才额外提供 ECM 与 ECM+BPF。BPF 只观察 CPU 可见 LAN 边缘流量；ECM 读取 NSS ECM node 累计值；ECM+BPF 把独立 aarch64 BPF kprobe 挂到 ECM totals-update 链路。它不是完整流量审计系统，旁路网关、同网段直连、桥内转发、驱动 offload、代理 TUN/IFB 等路径仍可能改变可见范围，因此不声明全流量绝对准确。
...
- **实时速率**：BPF 由 TC-BPF 按 MAC + zone/VLAN 计数；ECM 读取 ECM node advanced statistics；ECM+BPF 的 kprobe 只消费 `ecm_db_connection_data_totals_update` 的慢路径包增量和 NSS IPv4/IPv6 同步增量。三种口径不会互相相加，conntrack 始终不参与客户端总速率。
...
- **架构与 NSS 边界**：非 NSS 设备由 BPF tc 提供客户端速率；x86_64 的自动模式只能落到 BPF，配置页只列出自动与 BPF，也不会构建、安装或探测 ECM 对象。Qualcomm aarch64 NSS 设备自动按 ECM+BPF、ECM、BPF 的顺序选择。ECM+BPF 从同一个 ECM 更新函数同时覆盖慢路径与 NSS 同步增量，使用连接指针、`uint32_t time_added`、方向和客户端 MAC 区分流代次；它不读取 TC map 或 ECM node totals 做二次叠加。
- **活跃客户端**：默认只把 10 秒内仍有有效速率的客户端计为 active，可通过 UCI 调整。
- **覆盖率**：NSS 模式把物理 LAN 包时钟作为独立可信度窗口。原始客户端字节或包数暂时领先 LAN 时只让覆盖率进入 `pending`，不会阻塞逐客户端速率；首次不匹配起 5 秒仍未追平才把该覆盖率窗口标为 `counter_skew` 并重建基线。FCS 只按真实包数每包增加 4 字节。
...
| 值 | 行为 |
|---|---|
| `auto` | x86_64 只选择 BPF；Qualcomm aarch64 NSS 按 ECM+BPF、ECM、BPF 的顺序选择当前健康的数据源。 |
| `bpf` | 强制 TC-BPF；在 NSS 卸载活跃时仍可使用，但明确标记为 slow-path-only，不静默切换。 |
| `nss_ecm_node` | 强制 ECM node advanced statistics；state 不可读时明确失败。 |
| `nss_ecm_bpf` | 强制独立 ECM totals-update kprobe map；BTF、对象、挂载或 map 读取失败时明确失败。 |

conntrack 只用于连接数、逐连接详情和 IP 元数据，不是客户端总速率后备来源。

daemon 启动时立即由 Rust 扫描 `/proc/<pid>/comm`，之后至多每 5 秒扫描一次，只把精确名称为 `dae` 或 `daed` 的进程视为运行态。BPF 可把 LAN TC 程序从 Normal（pref `49152`）事务切换到 Early passthrough（pref `1`）。两个 TC 对象是 `/usr/lib/bpf/lanspeed-ebpf-kfunc` 与 `/usr/lib/bpf/lanspeed-ebpf-fallback`；aarch64 才额外安装 `/usr/lib/bpf/lanspeed-ebpf-ecm`，ECM+BPF 只从该对象加载 kprobe 和独立 map。

NSS collector 将 ECM state 输出掩码临时切到 node（`8`），读取 `from_data_total`、`to_data_total`、对应 packet total 与 `time_added`，打开字符设备后立即恢复原掩码。采集器不写 `defunct_all`、`flush` 或 `decelerate`。

ECM+BPF 启动时合并 `/sys/kernel/btf/vmlinux` 与 `/sys/kernel/btf/ecm`，解析 `ecm_db_connection_instance.node`、`ecm_db_connection_instance.time_added` 和 `ecm_db_node_instance.address` 的实机偏移，再挂到 `ecm_db_connection_data_totals_update`。该函数既接收 ECM 慢路径逐包更新，也接收 NSS IPv4/IPv6 同步差值，因此每个增量只进入一个 `lanspeed_ecm_clients` map；生产路径不会再读取 TC-BPF 或 ECM node 值相加。

首次快照保持 `warmup/0`。已有 node 的下一次增量立即按相邻 daemon 采样窗口发布；新出现的 node 只用首帧建立代际基线，下一帧增量即可发布。ECM 两次批次之间最多保留上一个速率 1.5 秒，随后明确归零，因此空闲数分钟后恢复流量也不会产生跨整段空闲期的低速平均值。独立 LAN 边界由已配置接口减去同样被配置的 bridge member 得到，因此 `br-lan + lan1/lan2/lan3` 不会重复计数。物理 LAN MIB 只负责覆盖率验证，不复制、缩放或阻塞客户端速率；两套窗口分别记录在 `evidence.nss_window`。
...
| `lanspeedd-bpf` | LuCI 应用的必选依赖；所有目标安装两套 TC-BPF 对象，aarch64 包才额外安装独立 ECM totals-update kprobe 对象；精确连接数统一来自 conntrack，并依赖 `lanspeedd` |
...
| 目标 | 说明 |
|---|---|
| `x86_64` LP64 | 支持 TC-BPF；不构建、不安装、不探测或显示 ECM 与 ECM+BPF。 |
| `aarch64` LP64 | 支持 TC-BPF；只有检测到 Qualcomm NSS/ECM 时才显示 ECM 与 ECM+BPF，后者使用以 `AYA_BPF_TARGET_ARCH=aarch64` 编译的独立对象。Release 分别提供 `aarch64_generic`、`aarch64_cortex-a53`、`aarch64_cortex-a72` 和 `aarch64_cortex-a76`，必须按固件包架构选择；交叉编译通过不等于具体设备已完成真机验证。 |
...
| kfunc TC、fallback TC 与 aarch64 ECM 三套 eBPF `build-std=core` 对象 | `1.87.0` | `1.97.1` | 每版独立构建目录；检查 TC/ECM 程序与 map 互斥、aarch64 kprobe 参数 ABI、EM_BPF、BTF、license section 及 32/64 位原子指令。 |
...
- `lanspeedd-bpf` 的标准 OpenWrt 包构建使用固定的 `bpf-linker 0.10.3`：x86_64 构建两套 TC-BPF 对象，aarch64 额外构建一套 ECM kprobe 对象；显式传入兼容版本的 `BPF_LINKER` 时，构建驱动按上述版本范围校验。目标机必须提供 `tc-full` 和 `kmod-sched-bpf`。统一使用多数流控插件采用的 `tc-full`，避免与 `tc-tiny` 的包冲突。
...
- NSS 自动模式优先使用独立 ECM+BPF，失败才选择 ECM node，再失败才以 slow-path-only 语义使用 BPF；手动模式不回退。它们都不替代 LuCI 应用对 `lanspeedd-bpf` 的安装依赖。
...
缺少 `lanspeedd-bpf`、tc 或内核 BPF 支持时属于不完整安装。非 NSS 设备实时速率不可用；NSS 设备的 ECM node 速率不依赖 BPF 是否成功挂载，但这不取代必选 BPF 依赖。
...
| `rate_collector_mode` | `auto` | 速率采集：x86_64 仅允许 `auto` / `bpf`；Qualcomm aarch64 NSS 允许 `auto` / `bpf` / `nss_ecm_node` / `nss_ecm_bpf`，自动优先级为 ECM+BPF、ECM、BPF。 |
...
| `enable_conntrack_fallback` | `1` | 是否允许 conntrack 连接数与逐连接详情回退；不改变客户端总速率所有权。 |
...
| `conn_source` | 实际连接元数据来源：`conntrack_netlink` / `conntrack_procfs` / `conntrack`。 |
...
| dae/daed | 代理接口不作为客户端身份，探测到时提示 `dae_detected`；BPF 可切到 Early passthrough；NSS 的 ECM 与 ECM+BPF 仍各自保持单一速率所有权。 |
...
| NSS ECM / PPE | ECM node 使用 node totals；ECM+BPF 独占 totals-update kprobe map 并覆盖 ECM 慢路径与 NSS/PPE 同步增量；BPF 只看慢路径，conntrack 只提供连接元数据。三者不相加。 |
...
| 缺少 BPF 包或对象 | `lanspeedd-bpf` 是 LuCI 应用的必选依赖；所有目标检查 `/usr/lib/bpf/lanspeed-ebpf-kfunc`、`/usr/lib/bpf/lanspeed-ebpf-fallback`，aarch64 NSS 的 ECM+BPF 另检查 `/usr/lib/bpf/lanspeed-ebpf-ecm`。 |
| 缺少 `tc` | 安装 `tc-full`。 |
| 连接数全 0 | 检查 `nf_conntrack_acct`、`kmod-nf-conntrack-netlink`、`conn_collector_mode`。 |
| 没有客户端 | 检查 LAN 接口配置、桥设备、BPF 是否 attach 成功。 |
| 速率长时间为 0 | NSS 设备确认 `effective_rate=nss_ecm_node`、ECM state 字符设备可读、node MAC 能映射客户端，并检查 `evidence.nss_window` 的 `state` / `reason` / 原始字节与包时钟。 |
| OpenClash 或 dae/daed 共存 | 非 NSS 检查 LAN BPF attach；NSS 检查 ECM node 状态，BPF 和 conntrack 字节不会与其相加。 |

...
try {
  ...
  assert(daemonVersion === '1.1.4', 'daemon PKG_VERSION must remain exactly 1.1.4 for this release');
  assert(luciVersion === '1.1.4', 'LuCI PKG_VERSION must remain exactly 1.1.4 for this release');
  ...
  assert(!readme.includes('GitHub Actions 在 `v*` tag 发布时'), 'README must not retain the obsolete tag-trigger description');
	assert(readme.includes('`1.1.4-r2`'), 'README full-version example must match the 1.1.4 release');
	assert(!/1\.1\.4-r(?!2(?:\D|$))\d+/.test(readme),
	  'README must keep the 1.1.4 release at r2');

	assert(daemonRelease === '2', 'daemon PKG_RELEASE must remain exactly 2');
	assert(luciRelease === '2', 'LuCI PKG_RELEASE must remain exactly 2');

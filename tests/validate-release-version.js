...
try {
  ...
  assert(readme.includes('`1.1.0-r4`'), 'README full-version example must match the 1.1.0 release');
  assert(!/1\.1\.0-r[5-9]/.test(readme), 'README must not advance the 1.1.0 release beyond r4');

  assert(daemonRelease === '4', 'daemon PKG_RELEASE must be exactly 4 for the automatic release workflow');
  assert(luciRelease === '4', 'LuCI PKG_RELEASE must be exactly 4 for the automatic release workflow');

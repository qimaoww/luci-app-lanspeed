...
function loadRefresh(vocabulary) {
  return vm.compileFunction(readModule('diagnosticsRefresh.js'),
    ...
    { filename: 'diagnosticsRefresh.js', parsingContext: context })(
      baseclass, format, vocabulary || vocab, { FULL_VERSION: '1.1.1-r9' }, statusCollector, model,
...
function loadView(rpc, shell, refresh, navigatorValue) {
  return vm.compileFunction(readModule('diagnosticsView.js'), [
    ...
    baseclass, rpc, { FULL_VERSION: '1.1.1-r9' }, model,
...
function healthyStatus(version = '1.1.1-r9') {
...
async function testStrictContracts() {
  ...
  const versionMismatch = payloads('1.1.1-r9');
  ...
  assert.strictEqual(model.versionStateWithRpc(mismatchState, mismatchState.status.version, '1.1.1-r9').state, 'warning');
...
async function testRequestOrdering() {
  ...
  const secondPayload = payloads('1.1.1-r9');
  ...
  assert.strictEqual(state.status.version, '1.1.1-r9');
  assert.strictEqual(state.diagnostics.versions.daemon, '1.1.1-r9');
...
async function testAlertsAndReport() {
  ...
  const deduplicatedReport = model.buildReport(duplicateState, '1.1.1-r9');
  ...
  const report = model.buildReport(state, '1.1.1-r9');
  ...
  const mapFailureReport = model.buildReport(mapFailureState, '1.1.1-r9');
  ...
  const failureReport = model.buildReport(secretFailure, '1.1.1-r9');

...
function loadRefresh(vocabulary) {
  ...
  return vm.compileFunction(readModule('diagnosticsRefresh.js'),
    ...
    { filename: 'diagnosticsRefresh.js', parsingContext: context })(
      baseclass, format, vocabulary || vocab, { FULL_VERSION: '1.2.1-r2' }, statusCollector, model,
...
function loadView(rpc, shell, refresh, navigatorValue, windowValue) {
  return vm.compileFunction(readModule('diagnosticsView.js'), [
    ...
    baseclass, rpc, { FULL_VERSION: '1.2.1-r2' }, model,
...
function healthyStatus(version = '1.2.1-r2') {
...
async function testStrictContracts() {
  ...
  const versionMismatch = payloads('1.2.1-r2');
  ...
  assert.strictEqual(model.versionStateWithRpc(mismatchState, mismatchState.status.version, '1.2.1-r2').state, 'warning');
...
async function testResourceStateMachine() {
  ...
  assert.strictEqual(model.integrityStateWithRpc(good).state, 'good');
	...
	const controlReport = model.buildReport(good, '1.2.1-r2');
...
async function testRequestOrdering() {
  ...
  const secondPayload = payloads('1.2.1-r2');
  ...
  assert.strictEqual(state.status.version, '1.2.1-r2');
  assert.strictEqual(state.diagnostics.versions.daemon, '1.2.1-r2');
...
async function testAlertsAndReport() {
  ...
  assert.strictEqual(new Set(Array.from(deduplicated.all, (item) => item.text)).size,
	...
	const deduplicatedReport = model.buildReport(duplicateState, '1.2.1-r2');
  ...
  const report = model.buildReport(state, '1.2.1-r2');
  ...
  const tcConflictReport = model.buildReport(tcConflictState, '1.2.1-r2');
  ...
  const mapFailureReport = model.buildReport(mapFailureState, '1.2.1-r2');
  ...
  const failureReport = model.buildReport(secretFailure, '1.2.1-r2');

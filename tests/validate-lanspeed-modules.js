...
function assertConfigSaveBehavior(configSrc, ifaceSrc) {
	...
	asyncChecks.push(Promise.resolve().then(function() {
		...
		const daemonControls = [
			...
			refs.activeMin, refs.showClientTotals, refs.showClientControl,
...
function assertStatusViewSourceOnlyState(src) {
	...
	if (!src.includes("showClientTotals: uciMain.show_client_totals !== '0'") ||
		...
		fail('LAN Speed status modules must load the remaining client display options');
	}
...
function assertStatusStyleModule(src) {
	...
	[
		...
		':is(:nth-child(3),:nth-child(4),:nth-child(5),:nth-child(6)){width:11%!important;text-align:center}'
	].forEach(function(marker) {
		if (!responsiveCss.includes(marker))
...
function assertStatusRefreshModule(src) {
	...
	if (src.includes('showClientStatus') || src.includes('setClientStatusVisibility') ||
		...
		!src.includes('controlCell.hidden = !showClientControl;')) {
		fail('lanspeed/statusRefresh.js must remove the obsolete client status column');
	}
...
function assertConfigModelRewrite(src) {
	...
	if (JSON.stringify(model.REMOVED_UCI_FIELDS) !== JSON.stringify([ 'dedicated_port', 'show_client_status' ]))
...
function makeConfigFormState(model, overrides) {
	...
	const numbers = [ 'refresh_interval_ms', 'active_client_window_ms', 'active_client_min_bps',
		...
		'nss_low_rate_high_watermark_bps', 'nss_fifo_target_delay_ms',
		'nss_fifo_min_queue_packets', 'rate_compensation_factor' ];
	const booleans = [ 'show_client_totals', 'show_client_control', 'show_ipv6', 'hide_private_ipv6',

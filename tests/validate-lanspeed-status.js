...
function normalizedResult(marker, at) {
	return {
		status: { marker: marker, version: '1.1.6-r3' },
		clients: { clients: [] },
...
async function testIndependentRpcSettlement(context, fmt) {
	...
	const rpc = {
		status: function() { return Promise.resolve({ version: '1.1.6-r3' }); },
...
function testPaginationAndUiStates(context, fmt) {
	...
	const state = {
		status: { version: '1.1.6-r3', coverage: { quality: 'idle' } },
	...
	assert.strictEqual(state.refs.meta.textContent, '后端 1.1.6-r3 · luci 1.1.6-r3');

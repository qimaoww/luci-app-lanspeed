...
function normalizedResult(marker, at) {
	return {
		status: { marker: marker, version: '1.1.5-r4' },
		clients: { clients: [] },
...
async function testIndependentRpcSettlement(context, fmt) {
	...
	const rpc = {
		status: function() { return Promise.resolve({ version: '1.1.5-r4' }); },
...
function testPaginationAndUiStates(context, fmt) {
	...
	const state = {
		status: { version: '1.1.5-r4', coverage: { quality: 'idle' } },
	...
	assert.strictEqual(state.refs.meta.textContent, '后端 1.1.5-r4 · luci 1.1.5-r4');

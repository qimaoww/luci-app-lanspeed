...
诊断页独立校验六个 RPC 请求。NSS 页面分开展示总速率 owner、精准接入拓扑和 NSS/CPU 分类；x86 页面只展示 TC-BPF 与连接详情健康。两种平台都会在健康诊断中读取本机全部 qdisc、class 与 filter，逐项显示接口、挂载位置、句柄、owner、统计和动作，并把普通共存与保留句柄占用、根队列占用、filter 抢先执行分开报告。

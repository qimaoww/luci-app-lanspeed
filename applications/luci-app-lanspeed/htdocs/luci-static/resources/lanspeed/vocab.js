...
var WARNING_LABELS = {
	...
	nss_ecm_node_parse_errors: _('NSS ECM 数据包含无法解析的记录，部分客户端速率可能缺失。'),
	nss_ecm_node_active: _('正在使用 NSS ECM node 的 MAC 聚合字节与真实包计数。'),
	nss_ecm_node_ecm_flows_only: _('ECM 模式只统计进入 ECM 数据库的连接，不会混入 TC-BPF 计数。'),
	nss_ecm_bpf_active: _('正在使用 ECM+BPF 更新链路统一采集慢路径与 NSS 加速增量。'),
	nss_ecm_bpf_disjoint_ownership: _('ECM+BPF 已隔离 NSS 硬件增量与 TC 慢路径增量，并在同一原始采样窗口计算一次速率。'),
	nss_ecm_bpf_tc_degraded: _('ECM+BPF 的 TC-BPF 补流不可用，当前仅使用 ECM 更新链。'),
	nss_ecm_bpf_runtime_unavailable: _('ECM+BPF 对象、BTF 布局、kprobe 或映射表尚未就绪。'),
	nss_ecm_bpf_map_read_failed: _('ECM+BPF 映射表读取失败，当前联合速率不可用。'),
	nss_bpf_slow_path_only: _('NSS 加速流量绕过 CPU BPF hook；BPF 只观测慢路径，不与 NSS 计数相加。'),
	pending: _('NSS 新批次尚未到达；上次速率只在一个同步周期内短暂保留。'),
	warmup: _('NSS 与 LAN 计数器正在建立冷启动基线。'),
	...
	conntrack_acct_disabled: _('Conntrack 计数未启用，连接详情可能不可用；NSS node 速率不依赖此计数。'),
	nf_conntrack_acct_disabled: _('nf_conntrack_acct 未启用，连接详情可能不可用；NSS node 速率不依赖此计数。'),
	...
	conntrack_not_sampled: _('NSS 速率路径不会在周期采集中读取 Conntrack；诊断请求会单独执行只读检查。'),
	conntrack_read_failed: _('Conntrack 读取失败；请检查 ctnetlink、nf_conntrack_netlink 和 Procfs 回退。'),
	...
	nss_control_not_configured: _('当前没有配置 NSS 客户端限速或禁网规则。'),
	nss_control_no_active_client: _('已保存 NSS 客户端控制规则，但当前没有可验证的在线客户端。'),
	nss_control_verification_pending: _('NSS 客户端控制已建立结构，正在等待实际路径和队列计数验证。'),
	nss_control_executor_failed: _('NSS 客户端控制的队列、分类器、nft 或路径验证失败。'),
	nss_client_control_unavailable: _('当前系统缺少 NSS 客户端控制所需的内核或用户态能力。'),
	nss_control_diagnostics_unavailable: _('尚未取得 NSS 客户端控制的完整诊断证据。'),
...
var IMPORTANT_WARNINGS = {
	...
	nss_ecm_node_parse_errors: true,
	nss_ecm_bpf_runtime_unavailable: true,
	nss_ecm_bpf_map_read_failed: true,
